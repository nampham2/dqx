...
def create_sample_graph() -> Graph:
    ...
    order_assertion1 = AssertionNode(
        actual=order_count > 0,
        label="Orders exist"
    )
    
    avg_order_value = sp.Symbol("avg_order_value")
    order_assertion2 = AssertionNode(
        actual=avg_order_value > 10,
        label="Average order value > $10"
    )
    
    order_assertion3 = AssertionNode(
        actual=avg_order_value < 1000,
        label="Average order value < $1000"
    )
    
    # Create customer assertions
    customer_count = sp.Symbol("customer_count")
    customer_assertion1 = AssertionNode(
        actual=customer_count > 0,
        label="Customers exist"
    )
    
    email_nulls = sp.Symbol("email_nulls")
    customer_assertion2 = AssertionNode(
        actual=email_nulls == 0,
        label="No null emails"
    )
    
    # Create revenue assertions
    daily_revenue = sp.Symbol("daily_revenue")
    revenue_assertion1 = AssertionNode(
        actual=daily_revenue > 10000,
        label="Daily revenue > $10k"
    )
    
    dod_change = sp.Symbol("dod_change")
    revenue_assertion2 = AssertionNode(
        actual=dod_change < 0.5,
        label="Day-over-day change < 50%"
    )
