...
class TestDataAvailabilityCalculation:
    ...
    def test_simple_metric_excluded_date(self) -> None:
        ...
        # Create a simple metric with lag=0
        sm = SymbolicMetric(
            name="num_rows()", symbol=sp.Symbol("x_1"), fn=lambda k: Success(100.0), metric_spec=specs.NumRows(), lag=0
        )
    ...
    def test_extended_metric_all_children_available(self) -> None:
        ...
        # Create two simple metrics
        sm1 = SymbolicMetric(
            name="sum(tax)", symbol=sp.Symbol("x_1"), fn=lambda k: Success(100.0), metric_spec=specs.Sum("tax"), lag=0
        )
        sm2 = SymbolicMetric(
            name="sum(tax)", symbol=sp.Symbol("x_2"), fn=lambda k: Success(110.0), metric_spec=specs.Sum("tax"), lag=1
        )
    ...
    def test_extended_metric_partial_availability(self) -> None:
        ...
        # Create three simple metrics for stddev
        sm1 = SymbolicMetric(
            name="average(price)",
            symbol=sp.Symbol("x_1"),
    ...
    def test_topological_sort_called(self) -> None:
        ...
        # Create metrics in wrong order (extended before simple)
        dod = SymbolicMetric(
            name="day_over_day(sum(revenue))",
            symbol=sp.Symbol("x_3"),
        ...
        sm1 = SymbolicMetric(
            name="sum(revenue)", symbol=sp.Symbol("x_1"), fn=lambda k: Success(100.0), metric_spec=specs.Sum("revenue")
        )
        sm2 = SymbolicMetric(
            name="sum(revenue)", symbol=sp.Symbol("x_2"), fn=lambda k: Success(110.0), metric_spec=specs.Sum("revenue")
        )
