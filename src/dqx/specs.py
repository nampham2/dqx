...
class MaxLength(SimpleMetricSpec):
    ...
    def __init__(
        ...
        self._column_type = column_type
        self._parameters = parameters or {}
        self._analyzers = (ops.MaxLength(self._column, self._column_type, parameters=self._parameters),)

    @property
    def name(self) -> str:
        return f"max_length_{self._column_type}({self._column})"
    ...
    def state(self) -> states.MaxLength:
        return states.MaxLength(value=self._analyzers[0].value())

    @classmethod
    def deserialize(cls, state: bytes) -> states.State:
        return states.MaxLength.deserialize(state)
