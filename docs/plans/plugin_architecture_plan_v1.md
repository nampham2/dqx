...
6. **Audit as Plugin**: Audit display is a built-in plugin that can be disabled by users
...
DQX includes a built-in audit plugin (`audit`) that displays colorful execution statistics using Rich tables in the console. It's enabled by default but can be disabled if needed.

To enable the audit plugin (it's enabled by default):
```python
from dataclasses import dataclass
from dqx.plugins import PluginConfig

@dataclass
class AuditPluginConfig(PluginConfig):
    enabled: bool = True
...
suite = VerificationSuite(
    ...
    name="My Suite",
    plugin_config={
        "audit": AuditPluginConfig(enabled=True)  # Or False to disable
    }
)
```

The audit plugin displays:
...
3. ✅ Built-in audit plugin that can be enabled/disabled
...
- A built-in audit plugin for tracking execution (enabled by default, can be disabled)
